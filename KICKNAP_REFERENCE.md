...
- **Service unit tests + a real prod bug fix (addendum):** added vitest to `services/availability` + `services/bookings`. 62 tests total across the repo (16 web + 17 availability + 9 bookings + 20 mobile, incl. 5 new mobile API-client tests with mocked fetch). Tests exposed a genuine bug in **`windowCovered`** (`services/availability/src/lib/time.ts`): it stepped the window in fixed +24h absolute intervals, so wall-clock days were mis-segmented — multi-day windows that start late in the day were wrongly **rejected** (Mon 22:00→Tue 14:00 with correct rules) and others wrongly **accepted** (Mon 18:00→Tue 04:00, ignoring Tuesday's 08:00 opening). Rewrote it to segment by true Amsterdam calendar days using a DST-safe midnight-of-date probe; **availability service redeployed** and the multi-day crossing case prod-verified (`outside_opening_hours` instead of a bad `available`). Also: gotcha — mobile had no vitest of its own (earlier `npm install apps\mobile -D vitest` had registered a `"mobile": "file:apps/mobile"` root devDep instead of installing into the app); removed that stray dep, installed vitest properly inside `apps/mobile`, added root `npm run test` that runs all four suites.
- Commits during this session: `e11c6e9` (unit tests + DST fix), `f4c3983` (mobile hardening), `8c4bb5e` (web cleanup), `1f21d6f` (win32 binding optional), `1dfaece` (gitignore `.env`), `d731faf` (mobile api tests), `38801ed` (service tests + `windowCovered` fix), plus root script/hygiene commit. Pushed together at wrap-up.
